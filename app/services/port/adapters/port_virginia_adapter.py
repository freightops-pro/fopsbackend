...
class PortVirginiaAdapter(PortAdapter):
    """Adapter for Port of Virginia API integration."""

    BASE_URL = "https://operations.portofvirginia.com/api"
    ...
    async def _make_request(self, endpoint: str, params: Optional[dict] = None) -> dict:
        """Make authenticated API request with API key."""
        if not self.api_key:
            raise PortAuthenticationError("Port of Virginia API key is required")
    ...
    async def track_container(self, container_number: str, port_code: str) -> ContainerTrackingResponse:
        """Track container using Port of Virginia import availability API."""
        try:
    ...
    async def get_container_events(
        ...
        """Get container event history from Port of Virginia container events API."""
        try:
            params = {"container_number": container_number}
            if since:
    ...
    async def get_vessel_schedule(self, vessel_name: Optional[str] = None, port_code: Optional[str] = None) -> list[dict]:
        """Get vessel schedule from Port of Virginia bookings API."""
        try:
            params = {}
            if vessel_name:
            ...
            data = await self._make_request("/bookings/vessel-schedules", params=params)
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            # Try to get vessel schedules as a test
