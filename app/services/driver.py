...
class DriverService:
    ...
    async def update_driver_profile(
        ...
        logger.info(f"DEBUG update_driver_profile: Driver {driver_id}, current email: {driver.email}, current user_id: {driver.user_id}")

        # Handle email updates
        if payload.email is not None:
            email_lower = payload.email.lower()
            logger.info(f"DEBUG: Updating email to {email_lower}")
            ...
            if driver.user_id:
                ...
                if user:
                    ...
                    logger.info(f"DEBUG: Updated existing user {user.id} email to {email_lower}")

            # If driver doesn't have a user account, create one
            else:
                logger.info(f"DEBUG: No user_id, checking for existing user with email {email_lower}")
                ...
                if existing_user:
                    # User exists! Link driver to this user
                    logger.info(f"DEBUG: Found existing user {existing_user.id} with email {email_lower}")
                    driver.user_id = existing_user.id
                else:
                    # Create new user account
                    logger.info(f"DEBUG: Creating new user for email {email_lower}")
                    temporary_password = self._generate_temporary_password()
                    ...
                    logger.info(f"DEBUG: Created user {user.id} and linked to driver")
                    ...
                    try:
                        ...
                        logger.info(f"DEBUG: Sent invitation email to {email_lower}")
                    except Exception as e:
                        logger.error(f"DEBUG: Failed to send invitation email: {e}")

            # Always update driver email
            driver.email = email_lower
        ...
        # Refresh driver to get updated user_id
        await self.db.refresh(driver)
        logger.info(f"DEBUG: After update - driver.user_id: {driver.user_id}")
    ...
    async def get_user_access(self, company_id: str, driver_id: str) -> UserAccessInfo:
        ...
        # DEBUG LOGGING
        logger.info(f"DEBUG get_user_access: Driver {driver_id}, email: {driver.email}, user_id: {driver.user_id}")
        
        # Try to find user by driver.user_id first
        if driver.user_id:
            ...
            if user:
                ...
                logger.info(f"DEBUG: Found user by user_id: {user.id}, status: {status}")
                ...
                logger.warning(f"DEBUG: driver.user_id {driver.user_id} exists but no User found!")
        
        # If no user_id or user not found, try to find user by email
        if driver.email:
            ...
            if user:
                # Found user by email! Link them to driver
                logger.info(f"DEBUG: Found user by email {email_lower}: {user.id}")
                logger.info(f"DEBUG: Linking driver {driver_id} to user {user.id}")
                
                driver.user_id = user.id
                try:
                    await self.db.commit()
                    logger.info(f"DEBUG: Successfully linked driver to user")
                except Exception as e:
                    logger.error(f"DEBUG: Failed to link driver to user: {e}")
                ...
                logger.info(f"DEBUG: No user found with email {email_lower}")
        
        # No user found at all
        logger.info(f"DEBUG: No user access found for driver {driver_id}")
        return UserAccessInfo(
            user_id=None,
            email=driver.email,
            status="none",
